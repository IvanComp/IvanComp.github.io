...
<div class="activities-page">
      ...
      {% for item in site.data.activities.organizing_committee.conference %}
        <li class="activity-item">
          <span class="activity-item__text">
            ...
            {% endif %}
            {% if item.role %}
              <span class="activity-item__organizers">{{ item.role | replace: "Ivan Compagnucci", "<strong>Ivan Compagnucci</strong>" }}</span>
            {% endif %}
  ...
  <section class="activity-section">
    ...
    <h3>Conference</h3>
    <ul class="activity-list">
      {% for item in site.data.activities.program_committee.conference %}
        <li class="activity-item">
          <span class="activity-item__text">
            <span class="activity-item__title">{{ item.title }}</span>
            {% if item.short_name %}
    ...
    <h3>Workshop</h3>
    <ul class="activity-list">
      {% for item in site.data.activities.program_committee.workshop %}
        <li class="activity-item">
          <span class="activity-item__text">
            <span class="activity-item__title">{{ item.title }}</span>
            {% if item.short_name %}
